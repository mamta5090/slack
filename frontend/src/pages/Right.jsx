...
const Right = () => {
  ...
  // socket is stored inside your socket slice as { socket, ... }
  ...
  // helper to create consistent room id between the two users
  const makeRoomId = (a, b) => [a, b].sort().join('_');

  // --- NEW: start call when clicking headphone icon ---
  const startCallFromIcon = () => {
    if (!socket || !user?._id || !singleUser?._id) {
      alert("Socket or user information not ready");
      return;
    }

    // deterministic room so both users join same room
    const roomID = makeRoomId(user._id, singleUser._id);

    // emit call-user payload to server
    const payload = {
      to: singleUser._id,
    ...
    // set calling state and a timeout
    setCalling(true);
  ...
  // Listen for call-answered and call-error events (forwarded to caller by server)
  useEffect(() => {
    if (!socket) return;

    const handleCallAnswered = (payload) => {
      // payload: { accepted, roomID, from }
      // server forwards only to the targeted caller, so it's safe to handle directly
      setCalling(false);
      ...
      <div className="flex-row flex items-center justify-between px-4 py-2 ">
        ...
        <div className="flex items-center gap-4 text-xl text-gray-600">
          <div className="flex flex-row border rounded-xl p-1 items-center gap-2">
            {/* HEADPHONE ICON: starts call */}
            <div className="hover:bg-[#f3f4f6] rounded-md p-1">
              <CiHeadphones
                className={`cursor-pointer hover:text-purple-600 transition-colors ${calling ? 'animate-pulse text-gray-500' : ''}`}
                onClick={startCallFromIcon}
                title={calling ? 'Calling...' : 'Start call'}
              />
            </div>
