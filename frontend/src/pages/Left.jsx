...
//         return;
//       }
//       console.log(":satellite_antenna: Sending token:", token.slice ? token.slice(0, 40) + "..." : token);
...
//     } catch (error) {
//       console.error(":x: Error fetching users:", error.response?.data || error.message);
...
const Left = () => {
  ...
  // :white_tick: hide the logged-in user
  ...
  return (
    <div
      ref={containerRef}
      className="w-[30%] h-[calc(100vh-48px)] bg-gray-700 overflow-y-auto no-scrollbar scroll-smooth ml-[72px] mt-[0px]"
    >
      <div className="font-bold text-lg text-white p-[10px] flex justify-between items-center">
        <div>Koalaliving</div>
        <div className="flex gap-[15px] text-xl">
