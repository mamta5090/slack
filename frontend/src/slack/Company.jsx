  ...
  useEffect(() => {
    const loadWorkspace = async () => {
      ...
      try {
        ...
        console.warn("no workspace from server (or endpoint not available)", err?.message);
      }
    };
...
const handleSaveCompany = async () => {
  try {
    ...
    alert(err?.response?.data?.message || err?.message || "Failed to save. See console.");
  }
};
        ...
        <div className="flex-grow overflow-auto flex items-start justify-center">
          <div className="w-full h-full bg-white p-10 shadow-lg">
            ...
            <div className="mb-8">
              <div className="relative">
                <input
                  type="text"
                  value={owners}
                  onChange={(e) => setOwners(e.target.value.slice(0, maxLength))}
            ...
            <div className="mb-8">
              ...
              <div className="text-gray-400 mb-4">Help your teammates to know they’re talking to the right person.</div>
  ...
  <img src={workspace.profileImage} alt="avatar" className="w-full h-full object-cover" />
) : (
  <span className="text-gray-500">No photo</span>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex gap-3">
                    <button onClick={onChoosePhoto} className="px-4 py-2 bg-gray-500 border border-[#2b3740] rounded-md text-sm text-white hover:bg-[#111827]">Upload photo</button>

                    {photoPreview && (
                      <button onClick={removePhoto} className="px-4 py-2 bg-transparent border border-[#2b3740] rounded-md text-sm text-gray-300 hover:bg-[#111827]">Remove</button>
                    )}
                  </div>
                  <div className="text-sm text-gray-400">Help your teammates confirm it’s you.</div>
                </div>
              </div>
            ...
            <div className="mt-6">
              <button
                ...
                className={`px-6 py-3 rounded-md font-semibold text-white ${owners.trim() ? "bg-[#6f2b72] hover:bg-[#5e265f]" : "bg-gray-500 text-gray-400 cursor-not-allowed"}`}
              >
                Next
              </button>
            </div>
